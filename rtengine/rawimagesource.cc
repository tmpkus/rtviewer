...
RawImageSource::RawImageSource(char * mfilename) :
	cache(NULL), border(4), RawImage(mfilename) {
	/*hrmap[0] = NULL;
	 hrmap[1] = NULL;
	 hrmap[2] = NULL;
	 needhr = NULL;
...
//%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
/*
 void RawImageSource::transformRect(PreviewProps pp, int tran, int &ssx1,
 int &ssy1, int &width, int &height, int &fw) {
...
//%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
/*
 void RawImageSource::getImage(ColorTemp ctemp, int tran, Imagefloat* image,
 ...
 min /= initialGain;
 //create 2 stop headroom when highlight recovery is enabled
 // this will be corrected again during color conversion
 headroom = (hrp.enabled&&(hrp.method !="Color"))? 4.0f : 1.0f;
 min *=headroom;
 printf(" headroom %f min %f\n",headroom,min);
 rm /= min;
 gm /= min;
 bm /= min;
 ...
 line_red[j] = rm * rd[jx];
 line_grn[j] = gm * gr[jx];
 line_blue[j] = bm * bl[jx];
 }
 else
 for (int j = 0, jx = sx1; j < imwidth; j++, jx += skip) {
 if (jx >= maxx - skip)
 jx = maxx - skip - 1; // avoid trouble
 line_red[j] = CLIP(rm*rd[jx]);
 line_grn[j] = CLIP(gm*gr[jx]);
 line_blue[j] = CLIP(bm*bl[jx]);
 }
 }
 } else {
 for (int j = 0, jx = sx1; j < imwidth; j++, jx += skip) {
 ...
 if (hrp.enabled)
 hlRecovery(hrp.method, line_red, line_grn, line_blue, i, sx1,
 imwidth, skip,headroom);

 transLine(line_red, line_grn, line_blue, ix, image, tran, imwidth,
 imheight, fw);
 }
 ...
 #endif
 if (fuji) {
 int a = ((tran & TR_ROT) == TR_R90 && image->width % 2 == 0) || ((tran
 & TR_ROT) == TR_R180 && image->height % 2 + image->width % 2
 == 1) || ((tran & TR_ROT) == TR_R270 && image->height % 2 == 0);
 // first row
 for (int j = 1 + a; j < image->width - 1; j += 2) {
 image->r[0][j] = (image->r[1][j] + image->r[0][j + 1]
 + image->r[0][j - 1]) / 3;
 image->g[0][j] = (image->g[1][j] + image->g[0][j + 1]
 + image->g[0][j - 1]) / 3;
 image->b[0][j] = (image->b[1][j] + image->b[0][j + 1]
 + image->b[0][j - 1]) / 3;
 }
 // other rows
 ...
 float dh = (ABS(image->r[i][j+1] - image->r[i][j-1])
 + ABS(image->g[i][j+1] - image->g[i][j-1])
 + ABS(image->b[i][j+1] - image->b[i][j-1])) / 1.0;
 float dv = (ABS(image->r[i+1][j] - image->r[i-1][j])
 + ABS(image->g[i+1][j] - image->g[i-1][j])
 + ABS(image->b[i+1][j] - image->b[i-1][j])) / 1.0;
 float eh = 1.0 / (1.0 + dh);
 float ev = 1.0 / (1.0 + dv);
 image->r[i][j] = (eh
 * (image->r[i][j + 1] + image->r[i][j - 1]) + ev
 * (image->r[i + 1][j] + image->r[i - 1][j])) / (2.0
 * (eh + ev));
 image->g[i][j] = (eh
 * (image->g[i][j + 1] + image->g[i][j - 1]) + ev
 * (image->g[i + 1][j] + image->g[i - 1][j])) / (2.0
 * (eh + ev));
 image->b[i][j] = (eh
 * (image->b[i][j + 1] + image->b[i][j - 1]) + ev
 * (image->b[i + 1][j] + image->b[i - 1][j])) / (2.0
 * (eh + ev));
 }
 // first pixel
 if (2 - (a + i + 1) % 2 == 2) {
 image->r[i][0] = (image->r[i + 1][0] + image->r[i - 1][0]
 + image->r[i][1]) / 3;
 image->g[i][0] = (image->g[i + 1][0] + image->g[i - 1][0]
 + image->g[i][1]) / 3;
 image->b[i][0] = (image->b[i + 1][0] + image->b[i - 1][0]
 + image->b[i][1]) / 3;
 }
 // last pixel
 if (2 - (a + i + image->width) % 2 == 2) {
 image->r[i][image->width - 1] = (image->r[i + 1][image->width
 - 1] + image->r[i - 1][image->width - 1]
 + image->r[i][image->width - 2]) / 3;
 image->g[i][image->width - 1] = (image->g[i + 1][image->width
 - 1] + image->g[i - 1][image->width - 1]
 + image->g[i][image->width - 2]) / 3;
 image->b[i][image->width - 1] = (image->b[i + 1][image->width
 - 1] + image->b[i - 1][image->width - 1]
 + image->b[i][image->width - 2]) / 3;
 }
 }
 // last row
 int b = (a == 1 && image->height % 2) || (a == 0 && image->height % 2
 == 0);
 for (int j = 1 + b; j < image->width - 1; j += 2) {
 image->r[image->height - 1][j] = (image->r[image->height - 2][j]
 + image->r[image->height - 1][j + 1]
 + image->r[image->height - 1][j - 1]) / 3;
 image->g[image->height - 1][j] = (image->g[image->height - 2][j]
 + image->g[image->height - 1][j + 1]
 + image->g[image->height - 1][j - 1]) / 3;
 image->b[image->height - 1][j] = (image->b[image->height - 2][j]
 + image->b[image->height - 1][j + 1]
 + image->b[image->height - 1][j - 1]) / 3;
 }
 }
 ...
 if (ri->isBayer() && skip==1 )
 correction_YIQ_LQ(image, raw.ccSteps);
 // Applying postmul
 colorSpaceConversion(image, cmp, embProfile, camProfile, xyz_cam, headroom);
 // Color correction
...
int RawImageSource::cfaCleanFromMap(PixelsMap &bitmapBads) {
	...
	for (int row = 2; row < H - 2; row++) {
		for (int col = 2; col < W - 2; col++) {
			...
			float wtdsum = 0, norm = 0, sum = 0, tot = 0;
			for (int dy = -2; dy <= 2; dy += 2) {
				for (int dx = -2; dx <= 2; dx += 2) {
					...
					float dirwt = 1 / (fabs(rawData[row + dy][col + dx]
									- rawData[row - dy][col - dx]) + eps);
			...
			if (norm > 0.0) {
				...
				if (tot > 0.1)
				rawData[row][col] = sum / tot;//backup plan -- simple average
			}
		}
	}
	return counter;
		...
		for (int rr = 0; rr < H; rr++) {
			...
			for (int cc = 0; cc < W; cc++) {
				...
				for (int nn = left; nn <= right; nn++) {
					hfnbrave
					+= fabs(rawData[mm][nn] - cfablur[mm * W + nn]);
				}
				hfnbrave = (hfnbrave - pixdev) / ((bottom - top + 1) * (right
								- left + 1) - 1);
...
//%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
/*
 void RawImageSource::rotateLine(float* line, float** channel, int tran, int i,
 int w, int h) {
...
//%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
/*
 void RawImageSource::transLine(float* red, float* green, float* blue, int i,
 Imagefloat* image, int tran, int imwidth, int imheight, int fw) {
 ...
 image->r[image->height - 1 - y][image->width - 1 - x]
 = red[j];
 image->g[image->height - 1 - y][image->width - 1 - x]
 = green[j];
 image->b[image->height - 1 - y][image->width - 1 - x]
 = blue[j];
 }
 }
 } else if ((tran & TR_ROT) == TR_R270) {
 ...
 image->g[row][col] = (green[j] + image->g[row + 1][col])
 / 2;
 ...
 int col = imwidth - 1 - j;
 image->r[row][col]
 = CLIP((int)(-0.0625*red[j] + 0.5625*image->r[row-1][col] + 0.5625*image->r[row+1][col] - 0.0625*image->r[row+3][col]));
 image->g[row][col]
 = CLIP((int)(-0.0625*green[j] + 0.5625*image->g[row-1][col] + 0.5625*image->g[row+1][col] - 0.0625*image->g[row+3][col]));
 image->b[row][col]
 = CLIP((int)(-0.0625*blue[j] + 0.5625*image->b[row-1][col] + 0.5625*image->b[row+1][col] - 0.0625*image->b[row+3][col]));
 }
 }
 } else if ((tran & TR_ROT) == TR_R90) {
 for (int j = 0; j < imwidth; j++) {
 ...
 for (int j = 0; j < imwidth; j++) {
 image->r[j][col]
 = CLIP((int)(-0.0625*red[j] + 0.5625*image->r[j][col-1] + 0.5625*image->r[j][col+1] - 0.0625*image->r[j][col+3]));
 image->g[j][col]
 = CLIP((int)(-0.0625*green[j] + 0.5625*image->g[j][col-1] + 0.5625*image->g[j][col+1] - 0.0625*image->g[j][col+3]));
 image->b[j][col]
 = CLIP((int)(-0.0625*blue[j] + 0.5625*image->b[j][col-1] + 0.5625*image->b[j][col+1] - 0.0625*image->b[j][col+3]));
 }
 }
 } else if ((tran & TR_ROT) == TR_R270) {
 for (int j = 0; j < imwidth; j++) {
 ...
 for (int j = 0; j < imwidth; j++) {
 int row = imwidth - 1 - j;
 image->r[row][2 * i - 1] = (red[j] + image->r[row][2 * i
 - 2]) / 2;
 image->g[row][2 * i - 1] = (green[j] + image->g[row][2 * i
 - 2]) / 2;
 image->b[row][2 * i - 1] = (blue[j] + image->b[row][2 * i
 - 2]) / 2;
 }
 } else if (i == imheight - 1) {
 for (int j = 0; j < imwidth; j++) {
 ...
 image->b[row][2 * i - 1] = (blue[j] + image->b[row][2 * i
 - 2]) / 2;
 image->r[row][2 * i - 3] = (image->r[row][2 * i - 2]
 + image->r[row][2 * i - 4]) / 2;
 image->g[row][2 * i - 3] = (image->g[row][2 * i - 2]
 + image->g[row][2 * i - 4]) / 2;
 image->b[row][2 * i - 3] = (image->b[row][2 * i - 2]
 + image->b[row][2 * i - 4]) / 2;
 }
 ...
 int row = imwidth - 1 - j;
 image->r[row][2 * i - 3]
 = CLIP((int)(-0.0625*red[j] + 0.5625*image->r[row][2*i-2] + 0.5625*image->r[row][2*i-4] - 0.0625*image->r[row][2*i-6]));
 image->g[row][2 * i - 3]
 = CLIP((int)(-0.0625*green[j] + 0.5625*image->g[row][2*i-2] + 0.5625*image->g[row][2*i-4] - 0.0625*image->g[row][2*i-6]));
 image->b[row][2 * i - 3]
 = CLIP((int)(-0.0625*blue[j] + 0.5625*image->b[row][2*i-2] + 0.5625*image->b[row][2*i-4] - 0.0625*image->b[row][2*i-6]));
 }
 }
 } else {
 ...
 if (i == 1 || i == 2) { // linear interpolation
 for (int j = 0; j < imwidth; j++) {
 image->r[2 * i - 1][j] = (red[j] + image->r[2 * i - 2][j])
 / 2;
 image->g[2 * i - 1][j]
 = (green[j] + image->g[2 * i - 2][j]) / 2;
 image->b[2 * i - 1][j] = (blue[j] + image->b[2 * i - 2][j])
 / 2;
 }
 } else if (i == imheight - 1) {
 for (int j = 0; j < imwidth; j++) {
 image->r[2 * i - 3][j] = (image->r[2 * i - 4][j]
 + image->r[2 * i - 2][j]) / 2;
 image->g[2 * i - 3][j] = (image->g[2 * i - 4][j]
 + image->g[2 * i - 2][j]) / 2;
 image->b[2 * i - 3][j] = (image->b[2 * i - 4][j]
 + image->b[2 * i - 2][j]) / 2;
 image->r[2 * i - 1][j] = (red[j] + image->r[2 * i - 2][j])
 / 2;
 image->g[2 * i - 1][j]
 ...
 for (int j = 0; j < imwidth; j++) {
 image->r[2 * i - 3][j]
 = CLIP((int)(-0.0625*red[j] + 0.5625*image->r[2*i-2][j] + 0.5625*image->r[2*i-4][j] - 0.0625*image->r[2*i-6][j]));
 image->g[2 * i - 3][j]
 = CLIP((int)(-0.0625*green[j] + 0.5625*image->g[2*i-2][j] + 0.5625*image->g[2*i-4][j] - 0.0625*image->g[2*i-6][j]));
 image->b[2 * i - 3][j]
 = CLIP((int)(-0.0625*blue[j] + 0.5625*image->b[2*i-2][j] + 0.5625*image->b[2*i-4][j] - 0.0625*image->b[2*i-6][j]));
 }
 }
 }
 }
...
void RawImageSource::getFullSize(int& w, int& h, int tr) {
	...
	/* rotation should really be done elsewhere.
...
int RawImageSource::load(string fname, bool batch) {
	...
	border = 8;
	if (ri->get_profile())
	embProfile = cmsOpenProfileFromMem(ri->get_profile(),
			ri->get_profileLen());
...
void RawImageSource::preprocess(void) {
	/*
	 MyTime t1, t2;
	 t1.set();
	 ...
	 rid = dfm.searchDarkFrame(ri->get_maker(), ri->get_model(),
	 ri->get_ISOspeed(), ri->get_shutter(), ri->get_timestamp());
	 }
	...
	/*
	 ...
	 rif = ffm.searchFlatField(idata->getMake(), idata->getModel(),
	 idata->getLens(), idata->getFocalLen(), idata->getFNumber(),
	...
	std::list<badPix> *bp = dfm.getBadPixels(ri->get_maker(), ri->get_model(),
			std::string(""));
	if (bp) {
		totBP += bitmapBads.set(*bp);
		if (settings.verbose) {
			std::cout << "Correcting " << bp->size()
			<< " pixels from .badpixels" << std::endl;
		}
	}
	...
	bp = 0;
	if (raw.df_autoselect) {
		bp = dfm.getHotPixels(ri->get_maker(), ri->get_model(),
				ri->get_ISOspeed(), ri->get_shutter(), ri->get_timestamp());
	...
	if (bp) {
		...
		if (settings.verbose && bp->size() > 0) {
			std::cout << "Correcting " << bp->size()
			<< " hotpixels from darkframe" << std::endl;
		}
	}
	...
	if (raw.hotdeadpix_filt > 0) {
		...
		float varthresh =
		(20.0 * ((float) raw.hotdeadpix_thresh / 100.0) + 1.0);
	...
	if (raw.greenthresh || (((idata->getMake().size() >= 7
									&& idata->getMake().substr(0, 7) == "OLYMPUS"
									&& idata->getModel()[0] == 'E') || (idata->getMake().size() >= 9
									&& idata->getMake().substr(0, 7) == "Panasonic")) && raw.dmethod
					!= RAWParams::methodstring[RAWParams::vng4] && ri->isBayer())) {
		// global correction
		int ng1 = 0, ng2 = 0, i = 0;
...
#pragma omp parallel for default(shared) private(i) reduction(+: ng1, ng2, avgg1, avgg2)
		...
		float corrg1 = ((float) avgg1 / ng1 + (float) avgg2 / ng2) / 2.0
		/ ((float) avgg1 / ng1);
		float corrg2 = ((float) avgg1 / ng1 + (float) avgg2 / ng2) / 2.0
		/ ((float) avgg2 / ng2);
...
#pragma omp parallel for default(shared)
		...
		if (ri->ISGREEN(i, j)) {
			float currData;
			currData = (float) (rawData[i][j] * ((i & 1) ? corrg2
							: corrg1));
	...
	/*
	 ...
	 if (raw.ca_autocorrect || fabs(raw.cared) > 0.001 || fabs(raw.cablue)
	 > 0.001) {
	 if (plistener) {
	 ...
	 * subtract dark frame (if present) from current image and apply flat field correction (if present)
	 ...
	 void RawImageSource::copyOriginalPixels(const RAWParams &raw, RawImage *src,
	 RawImage *riDark, RawImage *riFlatFile) {
	 if (ri->isBayer()) {
	 //if (!rawData)
	 rawData(W,H);// = allocArray<float> (W, H);
	 if (riDark && W == riDark->get_width() && H == riDark->get_height()) {
	 for (int row = 0; row < H; row++) {
	 for (int col = 0; col < W; col++) {
	 rawData[row][col]
	 = MAX (src->data[row][col]+ri->get_black() - riDark->data[row][col], 0);
	 }
	 }
	 } else {
	 for (int row = 0; row < H; row++) {
	 ...
	 if (riFlatFile && W == riFlatFile->get_width() && H
	 == riFlatFile->get_height()) {
	 ...
	 if (raw.ff_BlurType
	 == RAWParams::ff_BlurTypestring[RAWParams::v_ff])
	 cfaboxblur(riFlatFile, cfablur, 2 * BS, 0);
	 else if (raw.ff_BlurType
	 == RAWParams::ff_BlurTypestring[RAWParams::h_ff])
	 cfaboxblur(riFlatFile, cfablur, 0, 2 * BS);
	 else if (raw.ff_BlurType
	 == RAWParams::ff_BlurTypestring[RAWParams::vh_ff])
	 ...
	 float refctrval, reflocval, refcolor[2][2], vignettecorr,
	 colorcastcorr;
	 //find center ave values by channel
	 for (int m = 0; m < 2; m++)
	 for (int n = 0; n < 2; n++) {
	 refcolor[m][n]
	 = MAX(0,cfablur[(2*(H>>2)+m)*W+2*(W>>2)+n] - ri->get_black());
	 }
	 ...
	 vignettecorr
	 = (refcolor[m][n]
	 /MAX(1e-5,cfablur[(row+m)*W+col+n]-ri->get_black()));
	 rawData[row + m][col + n] = (rawData[row + m][col
	 + n] * vignettecorr);
	 }
	 }
	 ...
	 hlinecorr
	 = (MAX(1e-5,cfablur[(row+m)*W+col+n]-ri->get_black())
	 / MAX(1e-5,cfablur1[(row+m)*W+col+n]-ri->get_black()));
	 vlinecorr
	 = (MAX(1e-5,cfablur[(row+m)*W+col+n]-ri->get_black())
	 / MAX(1e-5,cfablur2[(row+m)*W+col+n]-ri->get_black()));
	 rawData[row + m][col + n]
	 = (rawData[row + m][col + n]
	 * hlinecorr * vlinecorr);
	 }
	 ...
	 void RawImageSource::cfaboxblur(RawImage *riFlatFile, float* cfablur, int boxH,
	 int boxW) {
	 ...
	 temp[row * W + col] = (temp[row * W + col - 2] * len
	 + riFlatFile->data[row][col + boxW]) / (len + 1);
	 temp[row * W + col + 1] = (temp[row * W + col - 1] * len
	 + riFlatFile->data[row][col + boxW + 1]) / (len + 1);
	 len++;
	 }
	 for (int col = boxW + 2; col < W - boxW; col++) {
	 temp[row * W + col] = temp[row * W + col - 2]
	 + ((float) (riFlatFile->data[row][col + boxW]
	 - riFlatFile->data[row][col - boxW - 2])) / len;
	 }
	 for (int col = W - boxW; col < W; col += 2) {
	 temp[row * W + col] = (temp[row * W + col - 2] * len
	 - riFlatFile->data[row][col - boxW - 2]) / (len - 1);
	 if ((W & 1) == 0)
	 temp[row * W + col + 1] = (temp[row * W + col - 1] * len
	 - riFlatFile->data[row][col - boxW - 1]) / (len - 1);
	 len--;
	 ...
	 cfablur[row * W + col] = (cfablur[(row - 2) * W + col] * len
	 + temp[(row + boxH) * W + col]) / (len + 1);
	 cfablur[(row + 1) * W + col] = (cfablur[(row - 1) * W + col] * len
	 + temp[(row + boxH + 1) * W + col]) / (len + 1);
	 len++;
	 }
	 for (int row = boxH + 2; row < H - boxH; row++) {
	 cfablur[row * W + col] = cfablur[(row - 2) * W + col] + (temp[(row
	 + boxH) * W + col] - temp[(row - boxH - 2) * W + col])
	 / len;
	 }
	 for (int row = H - boxH; row < H; row += 2) {
	 cfablur[row * W + col] = (cfablur[(row - 2) * W + col] * len
	 - temp[(row - boxH - 2) * W + col]) / (len - 1);
	 if ((H & 1) == 0)
	 cfablur[(row + 1) * W + col] = (cfablur[(row - 1) * W + col]
	 * len - temp[(row - boxH - 1) * W + col]) / (len - 1);
	 len--;
	 }
	 }
	 free(temp);
...
void RawImageSource::correction_YIQ_LQ_(Imagefloat* im, int row_from,
		int row_to) {
	...
	array2D<float> rbconv_Y (W,3), rbconv_I (W,3),
	rbconv_Q (W,3),rbout_I (W,3),rbout_Q (W,3);

	// TODO: these arrays could be converted much easier.
	...
	convert_row_to_YIQ(im->r[row_from - 1], im->g[row_from - 1], im->b[row_from
			- 1], rbconv_Y[px], rbconv_I[px], rbconv_Q[px], W);
	convert_row_to_YIQ(im->r[row_from], im->g[row_from], im->b[row_from],
			rbconv_Y[cx], rbconv_I[cx], rbconv_Q[cx], W);
	...
	for (int i = row_from; i < row_to; i++) {
		...
		if (i > row_from) {
			for (int j = 1; j < W - 1; j++) {
				row_I[j] = (rbout_I[px][j - 1] + rbout_I[px][j] + rbout_I[px][j
						+ 1] + rbout_I[cx][j - 1] + rbout_I[cx][j]
						+ rbout_I[cx][j + 1] + rbout_I[nx][j - 1]
						+ rbout_I[nx][j] + rbout_I[nx][j + 1]) / 9;
				row_Q[j] = (rbout_Q[px][j - 1] + rbout_Q[px][j] + rbout_Q[px][j
						+ 1] + rbout_Q[cx][j - 1] + rbout_Q[cx][j]
						+ rbout_Q[cx][j + 1] + rbout_Q[nx][j - 1]
						+ rbout_Q[nx][j] + rbout_Q[nx][j + 1]) / 9;
			}
			row_I[0] = rbout_I[px][0];
			row_Q[0] = rbout_Q[px][0];
			row_I[W - 1] = rbout_I[px][W - 1];
			row_Q[W - 1] = rbout_Q[px][W - 1];
			convert_row_to_RGB(im->r[i - 1], im->g[i - 1], im->b[i - 1],
					rbconv_Y[px], row_I, row_Q, W);
		}
	}
	// blur last 3 row and finalize H-1th row
	for (int j = 1; j < W - 1; j++) {
		row_I[j] = (rbout_I[px][j - 1] + rbout_I[px][j] + rbout_I[px][j + 1]
				+ rbout_I[cx][j - 1] + rbout_I[cx][j] + rbout_I[cx][j + 1]
				+ rbconv_I[nx][j - 1] + rbconv_I[nx][j] + rbconv_I[nx][j + 1])
		/ 9;
		row_Q[j] = (rbout_Q[px][j - 1] + rbout_Q[px][j] + rbout_Q[px][j + 1]
				+ rbout_Q[cx][j - 1] + rbout_Q[cx][j] + rbout_Q[cx][j + 1]
				+ rbconv_Q[nx][j - 1] + rbconv_Q[nx][j] + rbconv_Q[nx][j + 1])
		/ 9;
	}
	...
	convert_row_to_RGB(im->r[row_to - 1], im->g[row_to - 1], im->b[row_to - 1],
			rbconv_Y[cx], row_I, row_Q, W);
...
//%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
#if 1
void RawImageSource::colorSpaceConversion(Imagefloat* im,
		ColorManagementParams cmp, cmsHPROFILE embedded,
		cmsHPROFILE camprofile, float camMatrix[3][3], float colorscale) {

	//camMatrix is cam2xyz = xyz_cam

	if (cmp.input == "(none)")
	{
		// no profile but we still need to restore the scaling.
#pragma omp parallel for
	...
	if (0) /* (inProfile=="(camera)" ||inProfile=="" || (inProfile=="(embedded)" && !embedded))*/{
...
#pragma omp parallel for
		...
		for (int j = 0; j < im->width; j++) {
			...
			float newr = mat[0][0] * im->r[i][j] + mat[0][1] * im->g[i][j]
			+ mat[0][2] * im->b[i][j];
			float newg = mat[1][0] * im->r[i][j] + mat[1][1] * im->g[i][j]
			+ mat[1][2] * im->b[i][j];
			float newb = mat[2][0] * im->r[i][j] + mat[2][1] * im->g[i][j]
			+ mat[2][2] * im->b[i][j];
		...
		int blocksize = im->planestride / 24;
		int rest = im->planestride % 24;
		int do_gamma_trs=0;
		out = iccStore->workingSpace(cmp.working);
		lcmsMutex->lock();
		cmsHTRANSFORM hTransform = cmsCreateTransform(in, (FLOAT_SH(1)
						|COLORSPACE_SH(PT_RGB) | CHANNELS_SH(3) | BYTES_SH(4)
						|PLANAR_SH(1)), out, (FLOAT_SH(1) | COLORSPACE_SH(PT_RGB)
						|CHANNELS_SH(3) | BYTES_SH(4) | PLANAR_SH(1)),
				settings.colorimetricIntent, 0);
		lcmsMutex->unlock();
		if (hTransform) {//there is an input profile
			if (cmp.gammaOnInput) {do_gamma_trs=1;}
		} else {
			lcmsMutex->lock();
			hTransform = cmsCreateTransform(camprofile, (FLOAT_SH(1)
							|COLORSPACE_SH(PT_RGB) | CHANNELS_SH(3) | BYTES_SH(4)
							|PLANAR_SH(1)), out, (FLOAT_SH(1) | COLORSPACE_SH(PT_RGB)
							|CHANNELS_SH(3) | BYTES_SH(4) | PLANAR_SH(1)),
					settings.colorimetricIntent, cmsFLAGS_NOOPTIMIZE);
			lcmsMutex->unlock();
		}
		float * tempimage = new float[im->planestride * 3];
#pragma omp parallel
		{
#pragma omp for
			for (int t = 0; t < 24; t++) {
				int bz = (t < 24) ? blocksize : (blocksize + rest);
				float * dest = tempimage + t * blocksize * 3;
				float * src = im->data + t * blocksize;
				float div = 1.0 / 65535.0f;
				for (int j = 0; j < 3; j++) {
					for (int i = 0; i < bz; i++)
					dest[i] = src[i] * div;
					src += im->planestride;
			...
			if (do_gamma_trs) {
				 ...
				 */
#pragma omp  for
				for (int i = 0; i < im->planestride * 3; i++)
				tempimage[i] = CurveFactory::gamma(CLIP(tempimage[i]));
			}

#pragma omp  for
			for (int t = 0; t < 24; t++) {
				int bz = (t < 24) ? blocksize : (blocksize + rest);
				cmsDoTransform(hTransform, tempimage + t * blocksize * 3,
						tempimage + t * blocksize * 3, bz);
			}
...
#pragma omp  for
			for (int t = 0; t < 24; t++) {
				...
				float * src = im->data + t * blocksize;
				for (int j = 0; j < 3; j++) {
					// here we restore the 2 stop correction for highlights
					for (int i = 0; i < bz; i++)
					src[i] = dest[i] * 65535.0f * colorscale;
					src += im->planestride;
					dest += bz;
				}
			}
...
#else
void RawImageSource::colorSpaceConversion (Imagefloat* im, ColorManagementParams cmp, cmsHPROFILE embedded, cmsHPROFILE camprofile, float camMatrix[3][3], float& defgain) {
...
void RawImageSource::colorSpaceConversion16(Image16* im,
		ColorManagementParams cmp, cmsHPROFILE embedded,
		cmsHPROFILE camprofile, float camMatrix[3][3], float& defgain) {
	...
	if (inProfile == "(camera)" || inProfile == "" || (inProfile
					== "(embedded)" && !embedded)) {
		// in this case we avoid using the slllllooooooowwww lcms
...
#pragma omp parallel for
		...
		//        out = iccStore->workingSpaceGamma (wProfile);
		lcmsMutex->lock();
		cmsHTRANSFORM hTransform = cmsCreateTransform(in, TYPE_RGB_16_PLANAR,
				out, TYPE_RGB_16_PLANAR, settings.colorimetricIntent, 0);
		lcmsMutex->unlock();
		if (hTransform) {
			if (cmp.gammaOnInput) {
				float gd = pow(2.0, defgain);
...
#pragma omp parallel for
			...
			lcmsMutex->lock();
			hTransform = cmsCreateTransform(camprofile, TYPE_RGB_16_PLANAR,
					out, TYPE_RGB_16_PLANAR, settings.colorimetricIntent, 0);
			lcmsMutex->unlock();
			cmsDoTransform(hTransform, im->data, im->data, im->planestride);
		}
...
void RawImageSource::HLRecovery_Luminance(float* rin, float* gin, float* bin,
...
void RawImageSource::HLRecovery_CIELab(float* rin, float* gin, float* bin,
		float* rout, float* gout, float* bout, int width, float maxval,
		float xyz_cam[3][3], float cam_xyz[3][3]) {
	...
	for (int i = 0; i < width; i++) {
		...
		if (r > maxval || g > maxval || b > maxval) {
			...
			float go = MIN (g, maxval);
			float bo = MIN (b, maxval);
			float yy = xyz_cam[1][0] * r + xyz_cam[1][1] * g + xyz_cam[1][2]
			* b;
			...
			float x = xyz_cam[0][0] * ro + xyz_cam[0][1] * go + xyz_cam[0][2]
			* bo;
			float y = xyz_cam[1][0] * ro + xyz_cam[1][1] * go + xyz_cam[1][2]
			* bo;
			float z = xyz_cam[2][0] * ro + xyz_cam[2][1] * go + xyz_cam[2][2]
			* bo;
			...
			float zr = (fz <= 0.206893) ? ((116.0 * fz - 16.0) / 903.3) : (fz
					* fz * fz);
			float xr = (fx <= 0.206893) ? ((116.0 * fx - 16.0) / 903.3) : (fx
					* fx * fx);
			x = xr * 65535.0;
			y = yy;
			z = zr * 65535.0;
			float rr = cam_xyz[0][0] * x + cam_xyz[0][1] * y + cam_xyz[0][2]
			* z;
			float gr = cam_xyz[1][0] * x + cam_xyz[1][1] * y + cam_xyz[1][2]
			* z;
			float br = cam_xyz[2][0] * x + cam_xyz[2][1] * y + cam_xyz[2][2]
			* z;
			rout[i] = (rr);
			gout[i] = (gr);
			bout[i] = (br);
...
void RawImageSource::hlRecovery(std::string method, float* red, float* green,
		float* blue, int i, int sx1, int width, int skip,float headroom) {

	if (method == "Luminance")
	HLRecovery_Luminance(red, green, blue, red, green, blue, width, 65535.0/headroom
	);
	else if (method == "CIELab blending")
	HLRecovery_CIELab(red, green, blue, red, green, blue, width, 65535.0/headroom
			, xyz_cam, cam_xyz);
...
int RawImageSource::getAEHistogram(LUTu & histogram, int& histcompr) {
	...
	for (int i = border; i < H - border; i++) {
		...
		for (int j = start; j < end; j++) {
			if (ri->ISGREEN(i, j))
			histogram[CLIP((int)(camwb_green*rawData[i][j]))
			>> histcompr] += 4;
			else if (ri->ISRED(i, j))
			histogram[CLIP((int)(camwb_red*rawData[i][j])) >> histcompr]
			+= 4;
			else if (ri->ISBLUE(i, j))
			histogram[CLIP((int)(camwb_blue*rawData[i][j]))
			>> histcompr] += 4;
		}
		else {
			for (int j = start; j < 3 * end; j++) {
				histogram[CLIP((int)(camwb_red*rawData[i][j+0])) >> histcompr]++;
				histogram[CLIP((int)(camwb_green*rawData[i][j+1])) >> histcompr]
				+= 2;
...
ColorTemp RawImageSource::getAutoWB() {
	...
	if (fuji) {
		for (int i = 32; i < H - 32; i++) {
			...
			for (int j = start; j < end; j++) {
				if (!ri->isBayer()) {
					float
					d =
					CLIP(initialGain*(ri->data[i][3*j]-cblack[0])*scale_mul[0]);
					if (d > 64000)
					continue;
					avg_r += d;
					rn++;
					d
					= CLIP(initialGain*(ri->data[i][3*j+1]-cblack[1])*scale_mul[1]);
					if (d > 64000)
					continue;
					avg_g += d;
					gn++;
					d
					= CLIP(initialGain*(ri->data[i][3*j+2]-cblack[2])*scale_mul[2]);
					if (d > 64000)
					continue;
					avg_b += d;
					bn++;
					...
					float
					d =
					CLIP(initialGain*(ri->data[i][j]-cblack[c])*scale_mul[c]);
					if (d > 64000)
					continue;
					float dp = d;
					if (c == 0) {
		...
		if (!ri->isBayer()) {
			...
			for (int j = 32; j < W - 32; j += 2) {
				...
				d[0][0]
				= CLIP(initialGain*(ri->data[i][j] -cblack[FC(i,j)])*scale_mul[FC(i,j)]);
				d[0][1]
				= CLIP(initialGain*(ri->data[i][j+1] -cblack[FC(i,j+1)])*scale_mul[FC(i,j+1)]);
				d[1][0]
				= CLIP(initialGain*(ri->data[i+1][j] -cblack[FC(i+1,j)])*scale_mul[FC(i+1,j)]);
				d[1][1]
				= CLIP(initialGain*(ri->data[i+1][j+1]-cblack[FC(i+1,j+1)])*scale_mul[FC(i+1,j+1)]);
				if (d[0][0] > 64000 || d[0][1] > 64000 || d[1][0] > 64000
						|| d[1][1] > 64000)
				continue;
				avg_r += d[ey][ex];
	...
	float reds = avg_r / rn * camwb_red;
	float greens = avg_g / gn * camwb_green;
	float blues = avg_b / bn * camwb_blue;

	float rm = rgb_cam[0][0] * reds + rgb_cam[0][1] * greens + rgb_cam[0][2]
	* blues;
	float gm = rgb_cam[1][0] * reds + rgb_cam[1][1] * greens + rgb_cam[1][2]
	* blues;
	float bm = rgb_cam[2][0] * reds + rgb_cam[2][1] * greens + rgb_cam[2][2]
	* blues;
...
void RawImageSource::transformPosition(int x, int y, int tran, int& ttx,
		int& tty) {
...
ColorTemp RawImageSource::getSpotWB(std::vector<Coord2D> red, std::vector<
		Coord2D> green, std::vector<Coord2D>& blue, int tran) {
	...
	if (!ri->isBayer()) {
		...
		for (int i = 0; i < red.size(); i++) {
			...
			if (initialGain * (ri->data[yr][3 * xr] - cblack[0]) * scale_mul[0]
					> 52500 || initialGain * (ri->data[yg][3 * xg + 1]
							- cblack[1]) * scale_mul[1] > 52500 || initialGain
					* (ri->data[yb][3 * xb + 2] - cblack[2]) * scale_mul[2]
					> 52500)
			continue;
		...
		for (int i = 0; i < red.size(); i++) {
			...
			gloc /= gnbrs;
			bloc /= bnbrs;
			if (rloc * initialGain < 64000 && gloc * initialGain < 64000
					&& bloc * initialGain < 64000) {
				reds += rloc;
				greens += gloc;
				blues += bloc;
				rn++;
	...
	if (2 * rn < red.size()) {
		...
		float rm = rgb_cam[0][0] * reds + rgb_cam[0][1] * greens
		+ rgb_cam[0][2] * blues;
		float gm = rgb_cam[1][0] * reds + rgb_cam[1][1] * greens
		+ rgb_cam[1][2] * blues;
		float bm = rgb_cam[2][0] * reds + rgb_cam[2][1] * greens
		+ rgb_cam[2][2] * blues;
