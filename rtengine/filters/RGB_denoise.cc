...
static void Bilateral_HDR_Luma(HDRImage &ref, HDRImage &working, HDRImage &res,
		int radius, float gamma, float amount, float anti_dot,
		volatile int &early) {
	...
	for (int X = 0; (X < W * early); X += blocksize) {
		...
		for (y = 0; y < radius * early; y++) {
			...
			for (int x = X; x < w * early; x++) {
				...
				RGB_reduce(ref, working, res, amount, gamma, anti_dot, x, y, s2,
						e2, s1, e1);
			}
		}
		...
		for (; y < (h - radius - 1) * early; y++) {
			...
			if (X == 0)
				for (; x < radius * early; x++) {
					RGB_reduce(ref, working, res, amount, gamma, anti_dot, x, y,
...
// setup size and temporary storage.
	...
	if (props.early)
		Bilateral_HDR_Luma(src, temp1, src, 2, gammaw / 10.0f, lumaw, 1.0f,
				props.early);
